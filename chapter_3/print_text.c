/* This library includes the printf function to be used. */
#include <stdio.h>

/* Exercise: Print the following... */

/*
In C, lowercase letters are significant.
main is where program execution begins.
Opening and closing braces enclose program statements in a routine.
All program statements must be terminated by a semicolon.
*/

int main(void)
{
	printf("1. In C, lowercase letters are significant.\n");
	printf("2. Main is where program execution begins.\n");
	printf("3. Opening and closing braces enclose program ");
	printf("statements in a routine.\n");
	printf("4. All program statements must be terminated ");
	printf("by a semicolon.");
}
